...
public enum VeilMultiBind {
    LEGACY {
        ...
        public void bindTextures(int first, IntBuffer targets, IntBuffer textures) {
            ...
            for (int i = 0; i < textures.limit(); i++) {
                GlStateManager._activeTexture(GL_TEXTURE0 + first + i);
    ...
    public static VeilMultiBind get() {
        if (multiBind == null) {
            // 1.21.11's mixed RenderSystem/GL texture state can leave target-sensitive
            // texture binds stale when ARB_multi_bind is used for raw Veil GL textures.
            multiBind = LEGACY;
            Veil.LOGGER.info("Multi-Bind disabled, using legacy");
        }
        return multiBind;
    }
